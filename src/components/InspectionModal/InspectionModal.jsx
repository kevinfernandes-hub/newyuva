...
export function InspectionModal({ location, onClose }) {
  if (!location) return null;
  const unpermitted = location.permits?.filter((p) => p.status === 'unmatched') || [];
    ...
    <div className={styles.backdrop} onClick={onClose} role="dialog" aria-modal="true">
      <div className={styles.card} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.body}>
          <p><strong>Target Sector:</strong> {location.name} ({location.subtitle})</p>
          <p><strong>Coordinates:</strong> {location.coords}</p>
          <p style={{ marginTop: '8px' }}>
            <strong>Geospatial Delta:</strong> Color Diff {location.colorDiff.toFixed(2)}%, SSIM Structural Area {location.ssimArea.toFixed(2)}%.
          </p>
